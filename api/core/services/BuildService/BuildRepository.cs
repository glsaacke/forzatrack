...
namespace api.core.services.BuildService
{
    public class BuildRepository : IBuildRepository
    {
         public List<Build> GetAllBuilds(){
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                Build build = new Build
                {
                    ...
                    TopSpeed = Convert.ToDouble(row["userID"]),
                    ZeroToSixty = Convert.ToDouble(row["userID"]),
        ...
        public Build GetBuildByID(int id)
        {
            ...
            string stm = "SELECT buildID, user_id, car_id, rank, speed_st, handling_st, acceleration_st, launch_st, braking_st, offroad_st, top_speed, zero_to_sixty, deleted FROM Builds WHERE buildID = @id";
         ...
         public void UpdateBuild(Build build){
            ...
            string stm = $"UPDATE Builds SET user_id =  user_id, car_id = @car_id, rank = @rank, speed_st = @speed_st, handling_st = @handling_st, acceleration_st = @acceleration_st, launch_st = @launch_st, braking_st = @braking_st, offroad_st = @offroad_st, top_speed = @top_speed, zero_to_sixty = @zero_to_sixty WHERE buildID = @id";
            ...
            cmd.Parameters.AddWithValue("@top_speed", build.TopSpeed);
            cmd.Parameters.AddWithValue("@zero_to_sixty", build.ZeroToSixty);
            cmd.Parameters.AddWithValue("@id", build.BuildId);
